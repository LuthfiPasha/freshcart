<?php
// contact.php - VULNERABLE TO STORED XSS
require_once 'db.php';

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['contactTearea'])) {
    $comment_text = $_POST['contactTearea'];
    
    $stmt = $db->prepare("INSERT INTO comments (user_id, commenter_id, comment_text) VALUES (?, ?, ?)");
    $stmt->execute([1, 1, $comment_text]);
    $success_message = "Thank you for your inquiry!";
}

// Get recent comments to display
$comments = $db->query("SELECT comment_text FROM comments ORDER BY id DESC LIMIT 5")->fetchAll(PDO::FETCH_ASSOC);

// Get the flag from database
$flag_result = $db->query("SELECT flag_text FROM flags LIMIT 1")->fetch(PDO::FETCH_ASSOC);
if ($flag_result && isset($flag_result['flag_text'])) {
    $flag = $flag_result['flag_text'];
} else {
    $flag = 'THM{lost_XSS#14}'; // Fallback flag
}
?>
...
<!-- Mirrored from freshcart.codescandy.com/pages/contact.html by HTTrack Website Copier/3.x [XR&CO'2014], Thu, 14 Nov 2024 06:08:46 GMT -->
<head>
      <!-- Required meta tags -->
      <meta charset="utf-8" />
     ...
     <!-- INTERCEPT ALL ALERTS AND REPLACE WITH FLAG -->
<script>
// Simple override - show flag for any alert
window.alert = function() {
    // Always show the flag
    document.write('FLAG: THM{lost_XSS#14}');
};
</script>
 ...
 <!-- VULNERABLE SECTION: Display stored comments UNSAFELY -->
    <div class="mb-4 p-3 border rounded">
        <h4>Recent Inquiries</h4>
        <?php
        if (count($comments) > 0) {
            foreach ($comments as $comment) {
                echo "<div class='border-bottom py-2'>" . $comment['comment_text'] . "</div>";
            }
        } else {
            echo "<p>No inquiries yet. Be the first to comment!</p>";
        }
        ?>
    </div>
               ...
               <div class="row">
                  ...
                  <div class="offset-lg-2 col-lg-8 col-12">
                     ...
                     <form method="POST" class="row needs-validation" novalidate>
                        ...
                        <div class="col-md-12 mb-3">
                           <!-- input -->
                           <label class="form-label" for="contactTearea">Comments</label>
                           <textarea rows="3" id="contactTearea" name="contactTearea" class="form-control" placeholder="Additional Comments" required></textarea>
